...
import { BasicForm, useForm } from '@meng-ui/naive-ui-form'
...
const initColumns = ref(
  props.columns?.map((item: any) => {
    ...
    if (slot[item.key] && isFunction(slot[item.key])) {
      item.render = (row: any) => (slot[item.key] as Function)(row)
    }
    return item
  })
)
...
const newPagination = computed(() => {
  ...
  return {
    ...
    prefix: (info: any) => `每页${info.pageSize}条，共${info.itemCount}条`,
    onUpdatePage,
    onUpdatePageSize
  }
})
